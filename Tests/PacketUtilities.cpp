...
int SendDataToClients(int queue, char* data, int datasize, int packetsize)
{
    char** packetArray;
...
int SendPacketsToAllClients(int queue, char** packetArray, int totalPackets)
{
    ...
    Mesg* msg;              // Message used to find out the currently connected clients.

    msg = (Mesg*) malloc(sizeof(Mesg));

    //Grab all the clients that are currently connected.
    if(ReadMessage(queue, msg, 0) < 0)
    {
        printf("SendAllPackets: Unable to read the message.\n");
        return -1;
    }

    //Check if the number of sockets is proper.
    if(msg->num_sockets < 1 || msg->num_sockets > MAXCONNECTIONS)
    {
        printf("SendAllPackets: num_sockets error [num:%d].\n", msg->num_sockets);
        return -1;
    }

    while(curClient < msg->num_sockets)
    {
